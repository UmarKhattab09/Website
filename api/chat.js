...
`;
  ...
  try {
    ...
    if (!data?.candidates?.[0]?.content?.parts?.[0]?.text) {
      return res.status(500).json({ reply: "Something went wrong with Gemini API." });
    }
